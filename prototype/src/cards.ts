 ...
 *
 * Every card has the three layers from docs/design/00-principles.md:
 *  fiction (what the player sees) · effect (what visibly changes) · mechanic (the real call)
 */
...
export interface Card {
  ...
  /** For act.speak — what the Claw actually says this turn */
  utterance?: string;
  /** For time.branch — how far back to rewind, and what mood bias to apply */
  rewind?: { turns: number; newMood: string };
  /** For mind.know — a fact the Claw asserts into the scene, testable by the narrator */
  knowledge?: { claim: string; target: string };
  /** For place.bind — a durable scene fact written into the tree's facts map */
  bind?: { key: string; value: string };
  /** For memory.recall — which prior entry (by label) to resurface */
  recall?: { entryLabel: string; framing: string };
  /** For ward.vow — text of the constraint to add to the session's active vows */
  vow?: string;
}
...
export const DECK: Card[] = [
  ...
  },
  {
    ...
    recall: {
      ...
      framing: "the moment the Stranger turned — when the name found him and he said you had the lantern's patience",
    },
